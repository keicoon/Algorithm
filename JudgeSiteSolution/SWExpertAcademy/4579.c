...
int main(void)
{
	...
	*/
	for (test_case = 1; test_case <= T; ++test_case)
	{
		char str[21];
		scanf("%s", str);
		printf("#%d %s\n", test_case, is_palindrome(str) == 0 ? "Not exist" : "Exist");
	}
	return 0; //정상종료시 반드시 0을 리턴해야 합니다.
}

const char wildcard = '*';
char is_palindrome(char str[21])
{
	int l = (int)strlen(str);
	for(int i = 0, l_i = l - 1, n = l / 2; i < n; i++)
	{
        if(str[i] == wildcard || str[l_i - i] == wildcard)
        {
            return 1;
        }
		else if(str[i] != str[l_i - i])
		{
			return 0;
		}
	}
